...
export default class App extends Component {
  ...
  joinRoom(currentUser) {
    // let roomId = 7591800 <- this is the main test room
    let roomId
    currentUser.getJoinableRooms().then(rooms => {
      // try to find the general room in the rooms we're already part of
      currentUser.rooms.some(room => {
        if (room.name === 'general' && room.createdByUserId === 'brandon') {
          roomId = room.id
          return true
        }
      })
      // Try to find the general room if we're already not part of it
      if (!roomId)
